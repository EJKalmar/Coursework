...
// define floor coordinates, normal, and plane
...
// define wall coordinates, normal, and plane
...
// calculate local color at an intersect point
...
//determines whether or not the ray intersects the wall
bool intersectWall(vector3 origin, vector3 ray) {

	//find where the ray intersects the wall plane
	vector3 intersect = wall.intersect(origin, ray);

	//if it is inside the wall, return true
	if (intersect.x >= wallLimit1.x && intersect.x <= wallLimit3.x && intersect.y >= wallLimit1.y && intersect.y <= wallLimit3.y)
	else
		return false;
}

//determine whether or not the ray intersects the floor
bool intersectFloor(vector3 origin, vector3 ray) {

	//find where the ray intersects the floor plane
	vector3 intersect = floorDef.intersect(origin, ray);

	//if its inside the floor, return true
	if (intersect.x >= floorLimit1.x && intersect.x <= floorLimit3.x && intersect.z >= floorLimit1.z && intersect.z <= floorLimit3.z)
		return true;
	else
		return false;
}

//calculate and return the color at the intersection point by mixing the local colour with reflective or shadow colour.
//reflective colour is calculated recursively
vector3 rayTracer(vector3 intersect, vector3 incidentRay) {
	...
	// else if the ray hits the floor, return grey mixed with a reflection component
	//the reflection component is calculated recursively
		...
		if (intersectWall(intersect, lightPosition)) color = localClr.scalar(ambient_coef);//if the wall is blocking the light, mix in the shadow color
		else if (reflectiveClr.dot(vector3(1, 1, 1)) == 0.0) color = localClr;//otherwise if the reflective component is 0 then the color is the local color
...

{
	...
	int windowHandle = glutCreateWindow("TME4 Program 2");
