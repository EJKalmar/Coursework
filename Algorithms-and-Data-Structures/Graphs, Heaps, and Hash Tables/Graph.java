 ...
 * T00651562
 * June 6th, 2019
 * Original code by Java Foundations
 */
...
public class Graph<T> implements GraphADT<T>
{
    ...
    {
        if (indexIsValid(index))//if the index is valid
        {
            for (int i = 0; i < numVertices; i++)//for each vertex in the adjacency matrix
                for(int j=index;j<numVertices-1;j++)//remove edges shared with deleted vertex by shifting the array
                    adjMatrix[i][j]=adjMatrix[i][j+1];
                    
            for(int i=index;i<numVertices-1;i++)//starting at vertex's index
            {
                vertices[i]=vertices[i+1];//remove vertex from vertices[] by shifting array
                adjMatrix[i]=adjMatrix[i+1];//remove vertex from adjacency matrix by shifting the matrix
            }           
        }
        numVertices--;
        modCount++;
